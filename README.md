...
| `unity/Docs/` | Roadmap, role prompts, milestone plans, specifications, status, risk, ownership, and handoff records. |
...
The final user decision is recorded in `unity/Docs/Ownership_Decision_Record.md`.
...
3. Inspect every open PR for overlap, dependency, base branch, and shared-file locks.
4. Create one focused branch with exactly one primary owner mode.
...
7. Run exact relevant validation and report blocked checks honestly.
8. Rebase onto current `main` before final review.
9. Merge only after GPT disposition and any required Codex source-mode/user fidelity gate.
