...
- Kingdom Mode now opens over a runtime 2.5D board: orthographic camera, PC/mobile pan and zoom, realm-colored terrain, board frame, landmarks, roads, selectable building silhouettes, upgrade-state markers, selectable territory outposts with owner/bonus labels, level/status labels, and a Board View toggle that hides the dashboard for city inspection.
