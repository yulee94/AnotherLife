param(
    [Parameter(Mandatory = $true)][string]$RealmId,
    [Parameter(Mandatory = $true)][string]$SlicePath,
    [string]$ChampionVisualPrecursor = ""
)

Set-StrictMode -Version Latest
$ErrorActionPreference = "Stop"

# Validates a realm content-authoring slice (troops, champions, research, skills)
# against the canonical six-family content map and the non-fabrication rule.
#
# This is the realm-parameterized successor to Test-StoneholdContentAuthoringMap.ps1.
# The realm-agnostic checks (8 research + 4 skill name-only records, 4 TroopType
# absence anchors, provenance, strict JSON) are identical for every realm; only the
# realm identity and the optional champion visual-precursor text vary.
#
# Usage (from repo root):
#   pwsh tools/game-data/Test-RealmContentAuthoringMap.ps1 `
#       -RealmId stonehold `
#       -SlicePath unity/Docs/Narrative/GameData/stonehold-content-authoring-map.json `
#       -ChampionVisualPrecursor "Stonehold Vanguard"
#
# Checks:
#   - strict UTF-8 JSON (no BOM, single LF terminator, no duplicate properties)
#   - exact top-level and family shape
#   - realmId matches -RealmId
...
#   - optional champion visual-precursor text match (when -ChampionVisualPrecursor set)
...
# Resolve the slice path against the repo root when a relative path is supplied.
if ([System.IO.Path]::IsPathRooted($SlicePath)) {
    $slicePath = $SlicePath
}
else {
    $slicePath = Join-Path $repoRoot ($SlicePath -replace "/", "\")
}
...
function Assert-True {
    ...
    if (-not $Condition) {
        throw "Realm content-authoring-map validation failed: $Message"
    }
}
...
function Read-StrictJson {
    ...
    try {
        ...
        throw "Realm content-authoring-map validation failed: $Label strict JSON validation failed: $($_.Exception.Message)"
    }
    ...
    try {
        ...
        throw "Realm content-authoring-map validation failed: $Label JSON conversion failed: $($_.Exception.Message)"
    }
}
...
$slice = Read-StrictJson $slicePath "realm content-authoring map ($RealmId)"
...
Assert-True ($slice.realmId -ceq $RealmId) "realmId must be '$RealmId'"
...
Assert-True ([string]$championFamily.absence.visualPrecursor -ne "") "champions absence visualPrecursor must be non-empty"
if ($ChampionVisualPrecursor -ne "") {
    Assert-True ([string]$championFamily.absence.visualPrecursor -cmatch [regex]::Escape($ChampionVisualPrecursor)) "champions absence must name the '$ChampionVisualPrecursor' precursor"
}
...
Write-Output "PASS: '$RealmId' slice authored 8 research + 4 skill name-only records (verbatim, traceable), 0 troop/champion records with explicit not_authored_unavailable absence, all mechanics blocked_required, provenance verified"
