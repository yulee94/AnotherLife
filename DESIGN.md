...
- The next production building family is the four-realm `TownHall`, using the existing stable center slot as a hero-scale civic anchor rather than replacing the castle keep or landmark layer. The Stonehold production source and Level `1`/`6`/`10` graybox are approved; Eldergrove, Crownlands, and Umbral Town Hall sources remain pending. Working Level `10` labels guide visual production but do not establish narrative canon, powers, institutions, or gameplay.
