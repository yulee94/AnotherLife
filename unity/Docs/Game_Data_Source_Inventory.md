...
| Quests | Five runtime-created `QuestDefinition` objects in `LocalQuestService`; strict NVS-01 has its own canonical catalog and save aggregate | Local Q1–Q5 fallback is effective for the prototype; NVS-01 is independently strict and canonical only for `OMEN_1` | Quest copy belongs to Codex narrative/content; canonical NVS A1 packet is the retained source for that quest | Q1–Q5 none; NVS catalog `schemaVersion: 1`, packet `omen1-a1-2026-07-29-v003`; NVS progress version `1` | Q1–Q5 in memory; NVS artifact at `Assets/StreamingAssets/AL/Narrative/OMEN_1.catalog.json`; verified NVS progress in `SaveGameData.Nvs01Progress` | Q1–Q5 are regenerated; unknown saved quest rows survive but are filtered; NVS defaults neutrally for old saves and fails closed for malformed, forward, or wrong-packet progress | #128/#133; QuestDefinition safety is #156; NVS durability/save is #134/#137 |
...
| NVS contract/source/loader/runtime | `unity/Assets/AL/Scripts/Narrative/Nvs01/Contracts/Nvs01CatalogModels.cs`; `unity/Assets/AL/Scripts/Narrative/Nvs01/Nvs01CatalogValidator.cs`; `unity/Assets/AL/Scripts/Narrative/Nvs01/Nvs01CatalogLoader.cs`; `unity/Assets/AL/Scripts/Narrative/Nvs01/Nvs01QuestRuntime.cs`; `unity/Assets/AL/Scripts/Narrative/Nvs01/INvs01QuestRuntime.cs`; `unity/Assets/AL/Scripts/Narrative/Nvs01/Nvs01ProgressPersistence.cs`; `unity/Assets/AL/Scripts/Services/Local/ISaveGameCandidateStore.cs`; `unity/Assets/AL/Scripts/UI/Kingdom/Nvs01KingdomPresenter.cs`; `unity/Assets/AL/Scripts/UI/Kingdom/KingdomSceneController.cs`; `unity/Assets/AL/Scripts/ChampionMode/Narrative/Nvs01ChampionEncounterAdapter.cs`; `unity/Assets/AL/Editor/Narrative/ExportNvs01Catalog.cs` |
...
| Save/reference state | `unity/Assets/AL/Scripts/Data/Runtime/SaveGameData.cs`; `unity/Assets/AL/Scripts/Data/Runtime/BuildingState.cs`; `unity/Assets/AL/Scripts/Data/Runtime/WarmasterState.cs`; `unity/Assets/AL/Scripts/Data/Catalogs/SaveSemanticCandidateValidation.cs`; `unity/Assets/AL/Scripts/Core/Interfaces/IResearchService.cs`; `unity/Assets/AL/Scripts/Core/Interfaces/IQuestService.cs`; `unity/Assets/AL/Scripts/Core/Interfaces/ITerritoryService.cs`; `unity/Assets/AL/Scripts/Services/Local/LocalSaveGameService.cs` |
| Current validators/tests | `unity/Assets/AL/Editor/Validation/QuestDefinitionAssetAuthorityValidator.cs`; `unity/Assets/AL/Tests/EditMode/QuestDefinitionAssetAuthorityTests.cs`; `unity/Assets/AL/Tests/EditMode/QuestSaveCompatibilityTests.cs`; `unity/Assets/AL/Tests/EditMode/SaveSemanticCandidateValidationTests.cs`; `unity/Assets/AL/Tests/EditMode/SavePersistenceRegressionTests.cs`; `unity/Assets/AL/Tests/EditMode/Narrative/Nvs01CatalogTests.cs`; `unity/Assets/AL/Tests/EditMode/Narrative/Nvs01QuestRuntimeTests.cs`; `unity/Assets/AL/Tests/EditMode/Narrative/Nvs01KingdomPresenterTests.cs`; `unity/Assets/AL/Tests/EditMode/Narrative/Nvs01KingdomSceneWiringTests.cs`; `unity/Assets/AL/Tests/EditMode/BootloaderServiceStackIntegrityTests.cs`; `unity/Assets/AL/Tests/EditMode/RuntimeContractSmokeTests.cs` |
...
`SaveGameData` now carries explicit format, schema, and profile-initialization metadata. `LocalSaveGameService` supplies reviewed backward-compatible defaults, performs bounded semantic classification before activation, and preserves unknown/future data read-only. Definition-ID migrations and aliases remain under #137.

NVS-01 progress is stored in the versioned `SaveGameData.Nvs01Progress` aggregate. A missing field is the one approved neutral compatibility default: an older current-format save remains playable and writable without changing its bytes during load. Explicit null, authored state under neutral version `0`, malformed or runtime-inconsistent shape, forward progress version, and unsupported packet identity are not writable. Runtime mutations clone the published save, validate the expected snapshot, persist and verify through the existing transaction, and publish the verified clone only after success; duplicate commits also re-verify the canonical authority twice before reporting durable success. Consequence effects, artifact acquisition, affinity, Gold, quest completion, and chapter unlock remain intentionally empty until their production authorities are wired.
...
| NVS-01 | Catalog/runtime tests cover canonical bytes/hash/version, strict JSON/schema, references/state graph, immutability, loader result/lifetime, runtime transactions, adapter outcomes, backward-compatible save classification, atomic candidate publication, exact-reference preservation on failure, and production Kingdom reload. | Still a quest-specific contract, not the common #183 manifest/family catalog; Champion encounter/result delivery and the five consequence authorities remain sequenced work |
