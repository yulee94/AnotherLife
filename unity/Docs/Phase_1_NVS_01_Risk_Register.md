...
| R4 | Critical | NVS-01 consequences still lack one durable atomic/idempotent transaction | Gold, affinity, quest completion, artifact/Tear, chapter unlock, ledger, and notification domains are not composed through one accepted candidate transaction | GPT + Codex engineering | **Blocked — #133/#134 after foundations** |
...
| R18 | High | Green repository workflow can hide policy gaps | machine-readable policy is unused; diff failures fall back open; PR/push ranges, classifier coverage, action/permission hardening, transcripts, failure proofs, and protection are incomplete | Codex engineering + GPT + maintainer | **Active — #155 reopened** |
...
| R25 | Critical | Battle simulation/encounter state can grant invalid progression | empty/invalid armies, side-effectful simulation, weak encounter identity/lifecycle, and Champion direct mutation remain | Codex engineering | **Blocked/open — #174/#180/#178** |
| R26 | Critical | Boss rewards can be fabricated, duplicated, or partially committed | current service grants fallback item, saves credits before equipment, uses process-local randomness, exposes mutable inventory, and boss catch grants synthetic/double value | Codex engineering + GPT | **Active — #168; binding PR #230 contract merged** |
...
| R35 | Medium | Status/metadata can drift during concurrent merges | ten implementations merged and auto-closed issues while blocking source defects remained; PR #231 merged during active review | GPT + maintainer | **Mitigated by current-main post-merge audits and correction status refreshes** |
...
#183 boss/reward/equipment source
          +
#163 accepted no-save economy
          +
#137 accepted candidate ledger/outbox
          +
#180 typed encounter completion
          ↓
...
### 3.6 World/relationship/notification
...
No correction or planner may broaden into NVS, source authorship, balance, scenes, Android bridge, production save schema, or another designated shared file without a new declared dependency and lock.
...
3. Correct #163 before save transactions, progression, or rewards consume economy state.
...
7. Correct #178 before ShellFoundation Player packaging.
8. Correct #155 and prove intentional failures/branch protection.
...
11. Permit pure #177/#176/#172/#168 lanes without production integration or closure.
12. Keep #183, #223, #150, A1/G1/runtime, progression reconnection, persisted world/relationship/boss work, Android export, and release claims behind prerequisites.
```
