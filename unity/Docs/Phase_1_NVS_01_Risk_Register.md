...
| R6 | High | Android Unity bridge is a placeholder | `UnityView.kt` displays text rather than `UnityPlayer` | Embedded Android→Unity→Android loop is unproven | GPT + Codex | **Deferred** — #135; NVS-01 defaults to bounded standalone runtime path unless reprioritized |
