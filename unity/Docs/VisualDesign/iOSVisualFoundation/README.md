# Another Life — iOS visual foundation 05

This review prototype covers the opening iOS experience and the first realm-management loop: launch identity, realm selection, Champion creation, inner-realm arrival, all four domains, the outer-realm world map, player-castle inspection, timed scouting, and close battlefield entry.
...
The city-board **World** control opens the outer-realm map without changing the player's locked realm. Its current prototype terrain is an oversized 1,320 × 1,200 local sector inside the phone viewport: drag across blank terrain, use a mouse wheel, or focus the map and use the Arrow keys to explore; Home recenters it. The sector HUD shows ten nearby representative player castles against the 300+ world-scale target. Every castle is a selectable user entity with a castle name, owner, alliance tag, distance, and power. **Open player profile** now opens a dedicated castle dossier with ruler activity, alliance, castle level, power, distance, scouting state, and separate Scout and Attack actions. Scouting consumes the same single global march slot, follows an animated route, counts down once per second, and changes the march HUD into a persistent **Report ready** state. The completed report reveals wall, defender, and protection intelligence; acknowledging it keeps that verified intelligence attached to the player profile for the remainder of the visual session. Rally remains visibly unavailable until alliance membership and rally rules are defined. Production must populate these records from authoritative spatial/player data and recycle or cluster off-screen castles rather than render the entire world at once. The **Camera** control cycles Bird's-eye, Quarter, and 3D views while preserving pan position, selected player/target, bookmark, and active march. Food, stone, hostile, ruin, and home-city nodes remain independently selectable; their target sheet still shows type, level, distance, purpose, expected return, bookmark state, and a separate dispatch action. One march slot is available in this slice. An active march exposes its destination and second-by-second timer, blocks conflicting dispatches, animates its route, and applies the stated reward to the same realm treasury only when complete. Completed non-player expeditions also leave an acknowledgement report in the march HUD. **Domain** returns to the city with construction, report, scouting, and treasury state intact.
...
- Opening the player dossier left the march capacity at `0 / 1`; it showed GreenWarden, Thornspire, `[ROOT] Rootbound Circle`, Level 12, `27.6K` power, `0.9 km`, and unknown intelligence before scouting.
- Dispatching the 14-second Thornspire scout changed the dossier to `March slot occupied`, closed it without changing the selected player, animated the player-castle route, and counted the global march HUD from `1 / 1` to `Reports • 1`.
- The completed scout report showed Thornspire / GreenWarden, `27.6K` power, `21.4K` wall, `9.8K` defenders, and `Unshielded`; acknowledging it cleared the report badge and reopening the dossier showed `Verified report` plus `Rescout • 14s`.
- While that report remained unread, the dossier replaced Rescout with disabled `Review report first`, preserving the completed report instead of allowing a new dispatch to overwrite it.
- A complete 18-second Sunfield Grove gathering march added `+120 Food`, released the global march slot, and left a persistent expedition report whose acknowledgement restored the dispatch action.
- The dossier's Attack action opened the existing `Player siege` confirmation for Thornspire while preserving the Quarter camera and selected player.
- Camera cycling and direct drag preserved the selected player while coordinates changed from `X: 418 • Y: 726` to `X: 388 • Y: 740`.
...
- Player-dossier visual verdict passed at 96/100 and scout-report verdict passed at 94/100 against the approved World direction; remaining distance is intentional graybox character, enemy, and environment detail awaiting production asset approval.
