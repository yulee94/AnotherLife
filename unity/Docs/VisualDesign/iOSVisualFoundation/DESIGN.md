...
- Primary product surfaces: iOS launch, realm selection, Champion creation, arrival, kingdom management, outer-realm world map, player-castle dossier, timed scouting and march reports, attack confirmation, close-up Champion battlefield
...
- Core routes/screens: Launch → realm selection → Champion → arrival → Domain ↔ World → player dossier → timed scout → report → player dossier or attack confirmation → Battlefield → victory reward → World
...
- Player identity precedes conflict: opening a castle reveals its ruler, alliance, activity, level, power, distance, scouting state, and unavailable rally state before the separate Scout or Attack decision.
- Scouting earns intelligence: the player dossier begins with unknown wall, defender, and protection values; a timed scout occupies the global march slot, leaves a persistent completion report, and only then reveals those values in the dossier. An unread report blocks another scout or expedition so the evidence cannot be silently replaced.
...
- Success: Existing pack and construction outcomes remain exact; completed world marches clear the occupied slot, advance the chapter objective, report the named return, and add the reward to the same realm treasury; player scouting leaves a persistent report and changes the corresponding dossier from unknown to verified intelligence only after completion; each enemy reaches zero independently, battle victory occurs only when the living count reaches zero, and `Collect & Leave` applies the reward once before returning to the selected World target
...
- [ ] Define authoritative player-castle diplomacy, profile privacy, scouting outcomes, attack eligibility, rally, reinforcement, shield, relocation, alliance, and retry/reconciliation rules before the dossier and scout-report prototype become production behavior.
