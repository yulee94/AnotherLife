...
Status: C3 runtime migration for issue #180
...
`ChampionEncounterLoadGateway.Start` is the engine-free load/application entry. `ChampionEncounterProductionLoadPath.StartFromCommittedRealm` is the production start/load path. `ChampionEncounterRuntimeGateway.Apply` is the C3 runtime consumer of that C2 snapshot/receipt.
...
- #168 owns boss/reward definitions and duplicate-safe reward receipts. This gateway grants no reward and applies no economy value. C3 removes boss-side reward mutation from the encounter start/death path; C4 owns durable consequences.
...
C4 durable consequences and C5 presentation/route evidence remain later owned gates.
