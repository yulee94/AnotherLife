...
Generated blockout scenes intentionally remain outside production Build Settings. The concrete loader should be Unity Addressables 2.9.1 (or the project-approved later compatible version) so scene handles and dependencies have symmetric release semantics. That adapter and Addressables group mutation are deferred while the concurrent MVP thread owns `Packages/manifest.json`, `packages-lock.json`, and production build settings; the world catalog, coordinator, and scenes do not depend on those shared files.
