...
Owner-approved `DEC-SCENE-DELIVERY-001`, with the non-shipping inventory amendment `DEC-SCENE-DELIVERY-002`, pins Unity Addressables 2.9.1 and packages all 78 generated chunk scenes as local content in 11 world-instance groups, with exact addresses and fail-closed manifests documented in `Deterministic_Scene_Content_Delivery_Decision.md`. The generated scenes remain outside production Build Settings; remote catalogs and content-only updates remain disabled. This authoring/build configuration does not by itself make the current blockouts player-loadable: they still lack baked navigation data, and the runtime adapter remains `SceneManagerWorldChunkLoader`. A later `IWorldChunkLoader` migration may consume Addressables only if it preserves loader ownership, cancellation, visibility, actual-residency reporting, and symmetric operation-handle release.
