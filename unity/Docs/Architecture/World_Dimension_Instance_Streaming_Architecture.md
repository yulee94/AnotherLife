...
Generated blockout scenes intentionally remain outside production Build Settings. Owner-approved `DEC-SCENE-DELIVERY-001` pins Unity Addressables 2.9.1 and groups all 78 chunk scenes as local content by their 11 world instances, with exact addresses and fail-closed manifests documented in `Deterministic_Scene_Content_Delivery_Decision.md`. Remote catalogs and content-only updates remain disabled. Runtime loader wiring consumes this configuration through the existing `IWorldChunkLoader` boundary; it must retain Addressables operation handles and release them symmetrically.
