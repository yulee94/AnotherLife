#!/usr/bin/env python3
"""Fail-closed validation of UmbralConceptP1 V001 dimensions, hashes, links, and counts."""
...
SHEETS = [
    ("fam_umbral_capital", "umbral_capital_district_plan_v001"),
    ("fam_umbral_capital", "umbral_capital_skyline_north_south_v001"),
    ("fam_umbral_capital", "umbral_capital_skyline_east_west_v001"),
    ("fam_umbral_capital", "umbral_capital_keep_shell_v001"),
    ("fam_umbral_capital", "umbral_capital_keep_section_v001"),
    ("fam_umbral_capital", "umbral_capital_keep_ground_plan_v001"),
    ("fam_umbral_capital", "umbral_capital_keep_upper_circulation_v001"),
    ("fam_umbral_city_kit", "umbral_city_street_grammar_v001"),
    ("fam_umbral_city_kit", "umbral_city_dwelling_shell_v001"),
    ("fam_umbral_city_kit", "umbral_city_dwelling_interior_v001"),
    ("fam_umbral_city_kit", "umbral_city_market_service_public_hall_kit_v001"),
    ("fam_umbral_inner_cave_dungeon", "umbral_inner_cave_mouth_v001"),
    ("fam_umbral_inner_cave_dungeon", "umbral_inner_cave_section_circulation_v001"),
    ("fam_umbral_outer_cave_dungeon", "umbral_outer_cave_mouth_v001"),
    ("fam_umbral_outer_cave_dungeon", "umbral_outer_cave_loop_choke_section_v001"),
    ("fam_umbral_inner_cave_dungeon", "umbral_cave_chamber_fitting_module_v001"),
]
ALLOWED_CAPITAL = "capital_veilspire"
...
FORBIDDEN_P2 = (
    "gate_umbral_ashvein",
    "fam_umbral_sequential_gate",
    "fam_umbral_fortress",
    "fam_umbral_terrain",
    "fam_umbral_ecosystem",
)
...
def main() -> int:
    errors: list[str] = []
    manifest_path = ROOT / "umbral_concept_p1_manifest_v001.json"
    qa_path = ROOT / "visual_qa_v001.json"
    prompt_log_path = ROOT / "prompt_log_v001.json"
    ...
    if manifest.get("identity", {}).get("capitalId") != "capital_veilspire":
        fail(errors, "capitalId drifted from capital_veilspire")
    ...
    if manifest.get("dimensional_control_not_ai_numerals", {}).get("public_hall_opening_m") != [2.5, 3.0]:
    ...
    if not prompt_log_path.is_file():
        fail(errors, "missing prompt_log_v001.json")
    else:
        prompt_log = json.loads(prompt_log_path.read_text(encoding="utf-8"))
        sheets = prompt_log.get("sheets") or []
        if len(sheets) != 16:
            fail(errors, f"prompt_log sheets {len(sheets)} != 16")
        for row in sheets:
            if not row.get("prompt"):
                fail(errors, f"prompt_log missing prompt for {row.get('name')}")
            if "provider" not in row:
                fail(errors, f"prompt_log missing provider for {row.get('name')}")
    ...
    html = DOCS / "Umbral_Concept_P1_Packet_V001.html"
    md = DOCS / "Umbral_Concept_P1_Packet_V001.md"
    decision = DOCS / "Umbral_Concept_P1_Packet_V001_DECISION.md"
    for doc in (html, md, decision):
        if not doc.is_file():
            fail(errors, f"missing doc {doc.name}")
            continue
        text = doc.read_text(encoding="utf-8")
        if "capital_veilspire" not in text:
            fail(errors, f"{doc.name} missing capital_veilspire")
        ...
        for token in FORBIDDEN_P2:
            if token in text and "do not" not in text.lower():
                # allow mentioning exclusions; fail only if a P2 family is claimed as covered
                pass
        if "fam_umbral_terrain" in text or "fam_umbral_ecosystem" in text:
            fail(errors, f"{doc.name} must not cover P2 terrain/ecosystem families")
        if "fam_umbral_sequential_gate" in text or "fam_umbral_fortress_single_gate" in text:
            fail(errors, f"{doc.name} must not cover P2 gate/fortress families")

    if html.is_file():
        html_text = html.read_text(encoding="utf-8")
        for _, stem in SHEETS:
            rel = f"ArtSource/Environment/UmbralConceptP1/V001/{stem}.png"
