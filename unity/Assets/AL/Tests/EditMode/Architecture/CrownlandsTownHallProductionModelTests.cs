...
{
    public sealed class CrownlandsTownHallProductionModelTests
    {
        private const string PrefabPath =
            "Assets/AL/Art/Generated/Architecture/Crownlands/Production/" +
            "TownHall/Runtime/Crownlands_TownHall_Production.prefab";
        private const string CatalogPath =
            "Assets/AL/ScriptableObjects/Resources/" +
            "KingdomBuildingModelCatalog.asset";
        private const string AtlasPath =
            "Assets/AL/Art/Generated/Architecture/Crownlands/Production/" +
            "TownHall/Runtime/T_Crownlands_TownHall_Atlas_1024.png";
        private const string AtlasMaterialPath =
            "Assets/AL/Art/Generated/Architecture/Crownlands/Production/" +
            "TownHall/Runtime/MAT_Crownlands_TownHall_Atlas.mat";
        private const string AccentMaterialPath =
            "Assets/AL/Art/Generated/Architecture/Crownlands/Production/" +
            "TownHall/Runtime/MAT_Crownlands_TownHall_Accent.mat";
        ...
        public void PackagedCatalogDeclaresExactLiveTownHallBinding()
        {
            ...
            KingdomBuildingModelEntry entry = catalog.Entries.Single(
                candidate =>
                    candidate.RealmId == RealmId.Crownlands &&
        ...
        [TestCase(1, 9.40f, 5.00f, 9.13f)]
        [TestCase(6, 12.49f, 6.32f, 9.13f)]
        [TestCase(10, 12.49f, 8.24f, 10.06f)]
        ...
        public void LodTopologyMeetsFinalTriangleAndRendererBudgets()
        {
            ...
            int[] expectedTriangles = { 1584, 1412, 924, 652 };
        ...
        public void PrefabHasOnlyTheTwoApprovedRootBoxColliders()
        {
            ...
            AssertBox(
                ...
                true,
                new Vector3(0f, 4.25f, 0.1f),
                new Vector3(14f, 8.5f, 12.8f));
            AssertBox(
                model.NavigationCollider,
                false,
                new Vector3(0f, 0.9f, 0.1f),
                new Vector3(12.8f, 1.8f, 10.8f));
        }

        [Test]
        public void LevelTenConcordMeridianStaysShallowGroundedAndUnmarked()
        {
            GameObject prefab = LoadPrefab();
            ...
            Assert.That(
                TriangleCount(mesh),
                Is.EqualTo(516),
                "Two supports, ten beam segments, and one apex block " +
                "must remain the complete capstone.");
            Assert.That(mesh.bounds.max.y, Is.EqualTo(8.24f).Within(0.02f));
            Assert.That(mesh.bounds.size.x, Is.GreaterThan(8.2f));
            Assert.That(mesh.bounds.size.y, Is.LessThan(2.2f));
            Vector3[] supportVertices = mesh.vertices
                .Where(vertex =>
                    vertex.y <= 7.01f &&
                    Mathf.Abs(vertex.x) >= 3.9f)
                .ToArray();
            Assert.That(supportVertices, Is.Not.Empty);
            Vector3[] apexVertices = mesh.vertices
                .Where(vertex =>
                    vertex.y >= 7.45f &&
                    Mathf.Abs(vertex.x) <= 0.35f)
                .ToArray();
            Assert.That(
                apexVertices,
                Is.Not.Empty,
                "The shallow meridian must terminate in a solid apex block.");
            Assert.That(
                renderer.sharedMaterials.Any(material =>
                    AssetDatabase.GetAssetPath(material) ==
                    AccentMaterialPath),
                Is.False,
                "The apex stays unmarked and does not carry the accent.");
            Assert.That(
                mesh.bounds.center.z,
                Is.EqualTo(0.35f).Within(0.02f),
                "The meridian must stay directly over the grounded towers.");
        }

        [Test]
        public void LevelSixEstablishesExactlyTwoGroundedCivicTowers()
        {
            ...
            Assert.That(bounds.size.x, Is.GreaterThan(11f));
            Assert.That(bounds.max.y, Is.EqualTo(6.32f).Within(0.02f));
            Vector3[] towerCourseVertices = mesh.vertices
                .Where(vertex => vertex.y >= 5.6f)
                .ToArray();
            Assert.That(towerCourseVertices, Is.Not.Empty);
            Assert.That(
                towerCourseVertices.All(vertex =>
                    Mathf.Abs(vertex.x) >= 3.1f),
                Is.True,
                "The Level 6 upper course must belong only to the paired " +
                "grounded civic towers.");
        }
        ...
        public void ProductionPrefabOwnsStableCivicAnchors()
        {
            ...
            Assert.That(
                ...
                Is.EqualTo(new Vector3(0f, 0f, -5.55f)));
        }
        ...
        {
            var engineObject =
                new GameObject("CrownlandsProductionBinding.Engine");
            try
            {
                var engine =
        ...
        [Test]
        public void AdjacentConfirmedTownHallLevelUsesCrownlandsMotionOnce()
        {
            var engineObject =
                new GameObject("CrownlandsTownHallTransition.Engine");
            try
            {
                var engine =
                    engineObject.AddComponent<CityLayoutEngine>();
        ...
        {
            var engineObject =
                new GameObject("CrownlandsUpgradeBinding.Engine");
            try
            {
                var engine =
                    engineObject.AddComponent<CityLayoutEngine>();
        ...
        {
            var engineObject =
                new GameObject("CrownlandsUnbuiltBinding.Engine");
            try
            {
                var engine =
                    engineObject.AddComponent<CityLayoutEngine>();
        ...
        public void DeclaredInvalidBindingFailsVisiblyWithoutFallback()
        {
            ...
            var engineObject =
                new GameObject("CrownlandsInvalidBinding.Engine");
            try
            {
        ...
        public void MismatchedStableModelIdentityFailsVisibly()
        {
            var invalidCatalog =
                ScriptableObject.CreateInstance<
                    KingdomBuildingModelCatalog>();
            var engineObject =
                new GameObject("CrownlandsMismatchedBinding.Engine");
            try
            {
                invalidCatalog.Configure(
                    new[]
                    {
                        new KingdomBuildingModelEntry(
                            "building.crownlands.townhall.wrong",
                            RealmId.Crownlands,
                            "TownHall",
                            LoadPrefab(),
                            0.09f,
            ...
            BuildingState state)
        {
            return KingdomBuildingPresentationResolver
                .Resolve(RealmId.Crownlands, new[] { state })
                .Single(item => item.BuildingId == "TownHall");
        }
