...
{
    public sealed class UmbralTownHallLevelBlockoutTests
    {
        ...
        public void StableSpatialIdentityDoesNotMoveWithLevel(string prefabPath)
        {
            ...
            Assert.That(
                ...
                Is.EqualTo(new Vector3(0.2f, 3.55f, 0.1f)));
        ...
        public void ReviewAnchorsGrowMonotonicallyWithinGrayboxCeiling()
        {
            ...
            Assert.That(
                ...
                "Umbral Town Hall anchors exceed the static review ceiling.");
        }
        ...
        [Test]
        public void LevelOneSplitRoofRisesFromEavesToRidge()
        {
            GameObject prefab =
                AssetDatabase.LoadAssetAtPath<GameObject>(Level01PrefabPath);
            Transform west = prefab.transform.Find(
                "L01_OperationalHall/SplitCivicRoof_Occlusion/" +
                "WestCouncilRoofWestGroup/WestCouncilRoofWest");
            Transform east = prefab.transform.Find(
                "L01_OperationalHall/SplitCivicRoof_Occlusion/" +
                "WestCouncilRoofEastGroup/WestCouncilRoofEast");
        ...
        [TestCase(Level06PrefabPath)]
        [TestCase(Level10PrefabPath)]
        public void FourGroundedBoundaryPiersRemainCountable(string prefabPath)
        {
            GameObject prefab =
                AssetDatabase.LoadAssetAtPath<GameObject>(prefabPath);
            Transform piers = prefab.transform.Find(
                "L05_RealmStructure/FourGroundedBoundaryPiers");
            Assert.That(piers, Is.Not.Null);
            Assert.That(piers.childCount, Is.EqualTo(4));
            Assert.That(
                piers.Cast<Transform>()
            ...
            Assert.That(
                piers.Cast<Transform>().All(
                    child => child.Find("GroundedBase") != null &&
                             child.Find("ThickPierBody") != null &&
                             child.Find("BrassLoadCap") != null),
                Is.True);

            Transform shelter = prefab.transform.Find(
                "L05_RealmStructure/PhysicalShelterLoadPath");
            Assert.That(shelter, Is.Not.Null);
            Assert.That(shelter.childCount, Is.EqualTo(4));
            Assert.That(
                shelter.Cast<Transform>()
                    .Select(child => child.name)
                    .OrderBy(name => name),
                Is.EqualTo(new[]
                {
                ...
                "Boundary piers must brace into the roof, not form portals.");
        }

        [Test]
        public void VeiledAccordYokeIsGroundedStaticAndEmpty()
        {
            GameObject prefab =
                AssetDatabase.LoadAssetAtPath<GameObject>(Level10PrefabPath);
            Transform piers = prefab.transform.Find(
                "L05_RealmStructure/FourGroundedBoundaryPiers");
            Transform loadRails = prefab.transform.Find(
                "L10_VeiledAccordYoke/FourYokeLoadRails");
            Transform yoke = prefab.transform.Find(
                "L10_VeiledAccordYoke/VeiledAccordYoke_Occlusion");
            Assert.That(piers, Is.Not.Null);
            Assert.That(loadRails, Is.Not.Null);
            Assert.That(yoke, Is.Not.Null);
            Assert.That(loadRails.childCount, Is.EqualTo(4));
            Assert.That(
                loadRails.Cast<Transform>()
                    .Select(child => child.name)
                    .OrderBy(name => name),
                Is.EqualTo(new[]
                {
            ...
            {
                Transform frame = yoke.Find(frameName);
                Assert.That(frame, Is.Not.Null);
                Assert.That(frame.childCount, Is.EqualTo(4));
                Transform west = frame.Find("CouncilSlitBoundaryWest");
                Transform east = frame.Find("CouncilSlitBoundaryEast");
                Assert.That(west, Is.Not.Null);
                Assert.That(east, Is.Not.Null);
                Assert.That(
                    east.localPosition.x - west.localPosition.x,
                    Is.GreaterThan(0.7f),
                    "The upper council slit must remain empty negative space.");
            }
