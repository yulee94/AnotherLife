    ...
    public sealed class EldergroveWorkshopProductionModelTests
    {
        ...
        public void ProductionPrefabOwnsStableIdentityEnvelopeAndTenDeltas()
        {
            ...
            Assert.That(
                ...
                Is.EqualTo(new Vector3(9.2f, 6.8f, 7f)));
        ...
        [TestCase(1, 6.30f, 4.10f, 5.60f, 7.40f, 4.20f, 6.40f)]
        [TestCase(6, 7.77f, 4.91f, 5.60f, 9.00f, 5.10f, 6.80f)]
        [TestCase(10, 9.12f, 6.68f, 5.60f, 9.20f, 6.80f, 7.00f)]
            ...
            try
            {
                ...
                foreach (KingdomBuildingLevelDelta delta in model.LevelDeltas)
                {
                    ...
                    Assert.That(
                        concreteObjects,
                        Is.Not.Empty,
                        $"Level {delta.MinimumLevel} has no production mesh.");
                    Assert.That(
                        concreteObjects.All(
                            item =>
                                item.activeSelf ==
        ...
        public void LodTopologyMeetsFinalTriangleAndRendererBudgets()
        {
            ...
            int[] expectedRendererCeilings = { 10, 10, 10, 4 };
            ...
            for (int index = 0; index < lods.Length; index++)
            {
                ...
                Assert.That(
                    ...
                    Is.LessThanOrEqualTo(
                        expectedRendererCeilings[index]));
                triangles[index] = lods[index].renderers.Sum(
                    renderer =>
                        TriangleCount(
            ...
            Assert.That(triangles[0], Is.LessThanOrEqualTo(8000));
            Assert.That(
                triangles[1],
                Is.LessThanOrEqualTo(triangles[0] * 0.65f));
            Assert.That(
                triangles[2],
                Is.LessThanOrEqualTo(triangles[0] * 0.35f));
            Assert.That(
                triangles[3],
                Is.LessThanOrEqualTo(triangles[0] * 0.15f));
            Assert.That(triangles[1], Is.LessThan(triangles[0]));
            Assert.That(triangles[2], Is.LessThan(triangles[1]));
            Assert.That(triangles[3], Is.LessThan(triangles[2]));
            TestContext.Out.WriteLine(
                $"Eldergrove triangles: " +
                $"LOD0={triangles[0]}, LOD1={triangles[1]}, " +
                $"LOD2={triangles[2]}, LOD3={triangles[3]}.");
        }
        ...
        public void PrefabHasOnlyTheTwoApprovedRootBoxColliders()
        {
            ...
            AssertBox(
                ...
                new Vector3(9.4f, 6.8f, 7.2f));
            AssertBox(
                model.NavigationCollider,
                false,
                new Vector3(0f, 0.7f, 0f),
                new Vector3(8.6f, 1.4f, 6.2f));
        }
        ...
        {
            var engineObject =
                new GameObject("EldergroveUpgradeBinding.Engine");
            try
            {
                var engine =
                    engineObject.AddComponent<CityLayoutEngine>();
        ...
        {
            var engineObject =
                new GameObject("EldergroveUnbuiltBinding.Engine");
            try
            {
                var engine =
                    engineObject.AddComponent<CityLayoutEngine>();
        ...
        public void DeclaredInvalidBindingFailsVisiblyWithoutFallback()
        {
            ...
            var engineObject =
                new GameObject("EldergroveInvalidBinding.Engine");
            try
            {
        ...
        public void MismatchedStableModelIdentityFailsVisibly()
        {
            var invalidCatalog =
                ScriptableObject.CreateInstance<
                    KingdomBuildingModelCatalog>();
            var engineObject =
                new GameObject("EldergroveMismatchedBinding.Engine");
            try
            {
                invalidCatalog.Configure(
                    new[]
                    {
                        new KingdomBuildingModelEntry(
                            "building.eldergrove.workshop.wrong",
                            RealmId.Eldergrove,
                            "Workshop",
                            LoadPrefab(),
                            0.12f,
            ...
            BuildingState state)
        {
            return KingdomBuildingPresentationResolver
                .Resolve(RealmId.Eldergrove, new[] { state })
                .Single(item => item.BuildingId == "Workshop");
        }
